    ...
    module Actions
      module Volume
        module Default
          ...
          def volume_create(region:, label:, size:, image: nil,
                            snapshot_id: nil)
            action(:volume, :create, region: region, label: label, size: size,
                                     image: image, snapshot_id: snapshot_id)
          end
          ...
          def volume_attach(region:, volume_id:, vm_id:, target:)
            action(:volume, :attach, region: region, volume_id: volume_id,
                                     vm_id: vm_id, target: target)
          end
          ...
          def volume_extend(region:, volume_id:, size:)
            action(:volume, :extend, region: region, volume_id: volume_id,
                                     size: size)
          end

          def volume_snapshot_create(region:, volume_id:, label:)
            action(:volume, :snapshot_create, region: region,
                                              volume_id: volume_id,
                                              label: label)
          end

          def volume_snapshot_delete(region:, snapshot_id:)
            action(:volume, :snapshot_delete, region: region,

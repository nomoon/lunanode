      ...
      module Email
        module Default
          ...
          def email_domain_dkim_set(domain_id:, selector:, private_key:)
            action(:email, :domain_dkim_set, domain_id: domain_id,
                                             selector: selector,
          ...
          def email_user_add(domain_id:, username:, password:)
            action(:email, :user_add, domain_id: domain_id, username: username,
                                      password: password)
          end

          def email_user_set_password(domain_id:, user_id:, password:)
            action(:email, :user_set_password, domain_id: domain_id,
                                               user_id: user_id,
                                               password: password)
          end
          ...
          def email_alias_add(domain_id:, name:, target:)
            action(:email, :alias_add, domain_id: domain_id, name: name,
                                       target: target)
          end

          def email_alias_remove(domain_id:, alias_id:)
            action(:email, :alias_remove, domain_id: domain_id,
                                          alias_id: alias_id)
          end
        end
      end
